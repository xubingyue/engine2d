...
//texture
static int
calc_texture_size(enum TEXTURE_FORMAT format,int widht,int height){
    switch (format) {
        case TEXTURE_RGBA8:
            return widht * height *4;
        case TEXTURE_RGB565:
        case TEXTURE_RGBA4:
...
render_texture_subupdate(struct render *R,RID id,const void *pixels,int x,int y,int w,int h){
    ...
    int compressed = texture_format(tex, &format, &itype);
    if(compressed){
        glCompressedTexSubImage2D(GL_TEXTURE_2D, 0, x, y, w, h, format, calc_texture_size(tex->format, w, h), pixels);
    }else{
...
render_target_create(struct render *R,int width,int height,enum TEXTURE_FORMAT format){
    RID texid = render_texture_create(R, width, height, format, TEXTURE_2D, 0);
    if(texid == 0){
        return 0;
    }
    render_texture_update(R, texid, width, height, NULL, 0, 0);
    RID rt = create_rt(R, texid);
...
render_state_commit(struct render *R){
    ...
    if(R->changeflag & CHANGE_BLEND){
        if(R->last.blend_src != R->current.blend_src
           || R->last.blend_dst != R->current.blend_dst){
