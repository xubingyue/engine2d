...
void
texture_unload(int id_){
    if(id_ < 0 || id_ >= POOL.count){
        return;
    }
    struct texture *tex = &POOL.tex[id_];
    if(tex->id == 0){
        return;
    }
    //render_release
...
texture_active_rt(int id){
    ...
    render_set(R, TARGET, tex->fb, tex->id);
...
texture_update(int id,int pixel_width,int pixel_height,void *data){
    ...
    render_texture_update(R, id, pixel_width, pixel_height, data, 0, 0);
