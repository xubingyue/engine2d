...
struct sprite_trans *
sprite_trans_mul(struct sprite_trans *a,struct sprite_trans *b,struct sprite_trans *t,
...
sprite_child_visible(struct sprite *s,const char *childname){
    ...
    for (i=0; i<pf->n; i++) {
        ...
        int index = pp->component_id;
        struct sprite *child = s->data.children[index];
        if(child->name && strcmp(childname, child->name)){
            return true;
        }
    }
    return false;
...
static inline void
quad_aabb(struct pack_picture *pic ,struct srt *srt ,struct matrix *mat,int aabb[4]){
    int i;
    for (i=0; i<pic->n; i++) {
...
child_aabb(struct sprite *s,struct srt *srt ,struct matrix *mat,int aabb[4]){
    ...
    switch (s->type) {
        case TYPE_PICTURE:
            quad_aabb(s->s.pic, srt, mat, aabb);
            return 0;
        case TYPE_POLYGON:
            polygon_aabb(s->s.poly, srt, mat, aabb);
            return 0;
        case TYPE_LABEL:
            label_aabb(s->s.label, srt, mat, aabb);
            return 0;
        case TYPE_ANIMATION:
            break;
        case TYPE_PANNEL:
