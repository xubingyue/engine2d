...
local sw = fw.GameInfo.width
local sh = fw.GameInfo.height

local SKY_PCT = 0.6
local STAR_PCT = SKY_PCT * 0.9
local OBJ_ORB = 0.45
local OBJ_SCALE = fw.GameInfo.width / 768
...
function M:layout_sim(horz)
	if horz then
		sw = fw.GameInfo.height
		sh = fw.GameInfo.width
	else
		sw = fw.GameInfo.width
		sh = fw.GameInfo.height
	end
